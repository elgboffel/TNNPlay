...
namespace Umbraco.Web
{
    public static class UmbracoHelperExtensions
    {
        ...
        public static bool LeafletMapHasValues(this IPublishedContent content, string propertyName)
        {
            if (!content.HasProperty(propertyName))
                return false;

            var map = content.GetPropertyValue<LeafletMapModel>(propertyName);

            if (map == null || map.LatLng == null || map.LatLng.Lat == 0)
                return false;
